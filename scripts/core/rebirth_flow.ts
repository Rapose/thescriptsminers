...
async function main() {
  ...
  if (!fs.existsSync(".ess_mint.tmp")) {
    throw new Error(
      "ESS mint not found. Run scripts/core/create_fixed_mint.ts first."
    );
  }
  ...
  const unclampedLowBefore =
    parentHashSum > rebirthMin ? parentHashSum : rebirthMin;

  const expectedLowBefore =
    unclampedLowBefore > rebirthMax ? rebirthMax : unclampedLowBefore;
  ...
  const unclampedExpectedLow =
    parentA.hashBase + parentB.hashBase > minNext
      ? parentA.hashBase + parentB.hashBase
      : minNext;

  const expectedLow =
    unclampedExpectedLow > maxNext ? maxNext : unclampedExpectedLow;
