import { equipmentCtx, resolveMiner, resolveEssMint, findFreeFourSameLevel, pdas, fetchers, toNum, TOKEN_PROGRAM_ID, SystemProgram, getAccount, getOrCreateAssociatedTokenAccount, assertDefaultPubkey, assertEq, assertExists, assertFalse, assertPubkeyEq, assertTrue, printJson, printSuccess, printTx } from "./_common";

async function main() {
  const { env, args } = equipmentCtx("76-equipment-remelt-hand");
  ...
  const set = await findFreeFourSameLevel(env, owner, 0);
  ...
  const tx = await env.program.methods.equipmentRemeltHand().accounts({
    owner,
    equipmentCounter,
    outputEquipment,
  ...
  assertEq("output slot hand", Number(output.slot), 0);
  ...
  printTx("equipmentRemeltHand", tx);
  ...
  printSuccess("equipment_remelt_hand validado");
}
