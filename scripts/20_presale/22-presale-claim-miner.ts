...
async function main() {
  const { env, args } = presaleCtx("22-presale-claim-miner");
  ...
  const lootbox = pdas.lootboxMiner(env.programId, buyer, purchaseId, 1);

  const receiptBefore: any = await fetchers.fetchPresaleReceipt(env.program, receipt);
  const tx = await env.program.methods.presaleClaimMinerLootbox(bn(purchaseId)).accounts({
    buyer,
    config,
    receipt,
    lootbox,
  ...
  printTx("presaleClaimMinerLootbox", tx);
  ...
  printSuccess("Claim presale miner validado");
}
