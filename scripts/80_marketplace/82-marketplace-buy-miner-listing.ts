...
async function main() {
  const { env, args } = marketCtx("82-marketplace-buy-miner-listing");
  const essMint = resolveEssMint(args);
  const fixture = await fixtureParties(env, args, essMint);
  const listing = args.listing ? new PublicKey(args.listing) : await findActiveListingByKind(env, 1, fixture.seller);
  ...
  const minerProgress = pdas.minerProgress(env.programId, miner);
  ...
  const tx = await env.program.methods.marketplaceBuyMinerListing().accounts({
  ...
  assertPubkeyEq("owner do miner muda", minerAfter.owner, fixture.buyer.publicKey);
  ...
  printTx("marketplaceBuyMinerListing", tx);
  printJson("accounts", { listing: listing.toBase58(), miner: miner.toBase58(), seller: fixture.seller.toBase58(), buyer: fixture.buyer.publicKey.toBase58() });
  printSuccess("buy miner listing validado");
}
